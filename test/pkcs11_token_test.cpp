...
{
    ASSERT_NO_THROW({ Pkcs11Token token(softHsmPath, libresign::as_pin("1234"), "test-key", 0); });
}

TEST(Pkcs11TokenStandalone, FailsWithInvalidModule)
{
    ASSERT_THROW({ Pkcs11Token token("/nonexistent.so", libresign::as_pin("1234"), "key", 0); }, std::runtime_error);
}
