...
{
    auto bytes = LibreSCRS::SmartCard::Internal::findBytes(fields, tag);
    if (bytes.empty())
        return {};
...
{
    try {
        auto resp = conn.transmit(LibreSCRS::SmartCard::Internal::selectByAID(protocol::AID_SERVSZK));
...
std::vector<uint8_t> HealthCard::readFile(const std::vector<uint8_t>& fileId)
{
    ...
    auto selectResp = conn.transmit(LibreSCRS::SmartCard::Internal::selectByFileId(fileId[0], fileId[1]));
...
HealthDocumentData HealthCard::readDocumentData()
{
    ...
    auto docFields = LibreSCRS::SmartCard::Internal::parseTLV(docRaw.data(), docRaw.size());
    auto fixedFields = LibreSCRS::SmartCard::Internal::parseTLV(fixedRaw.data(), fixedRaw.size());
    auto varPersFields = LibreSCRS::SmartCard::Internal::parseTLV(varPersRaw.data(), varPersRaw.size());
    auto varAdminFields = LibreSCRS::SmartCard::Internal::parseTLV(varAdminRaw.data(), varAdminRaw.size());
    ...
    // Fixed personal file
    d.insurantNumber = LibreSCRS::SmartCard::Internal::findString(fixedFields, protocol::TAG_INSURANT_NUMBER);
    ...
    auto genderRaw = LibreSCRS::SmartCard::Internal::findString(varAdminFields, protocol::TAG_GENDER);
    if (genderRaw == "01")
        d.gender = "\xD0\x9C\xD1\x83\xD1\x88\xD0\xBA\xD0\xBE"; // Мушко
    else if (genderRaw == "02")
        d.gender = "\xD0\x96\xD0\xB5\xD0\xBD\xD1\x81\xD0\xBA\xD0\xBE"; // Женско
    else
        d.gender = genderRaw; // unknown code — pass through

    d.personalNumber = LibreSCRS::SmartCard::Internal::findString(varAdminFields, protocol::TAG_PERSONAL_NUMBER);
    ...
    auto taxId1 = LibreSCRS::SmartCard::Internal::findString(varAdminFields, protocol::TAG_TAXPAYER_ID_1);
    auto taxId2 = LibreSCRS::SmartCard::Internal::findString(varAdminFields, protocol::TAG_TAXPAYER_ID_2);
    d.taxpayerIdNumber = taxId1.empty() ? taxId2 : taxId1;

    d.taxpayerActivityCode = LibreSCRS::SmartCard::Internal::findString(varAdminFields, protocol::TAG_TAXPAYER_ACTIV);
