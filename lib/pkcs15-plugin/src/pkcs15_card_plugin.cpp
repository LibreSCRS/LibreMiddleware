...
Q_LOGGING_CATEGORY(lcPkcs15Plugin, "librescrs.pkcs15.plugin")
...
public:
    ...
    LibreSCRS::Plugin::CardFieldGroup readTokenInfo(LibreSCRS::SmartCard::CardSession& session) const override
    {
        ...
        try {
            auto holderResult = acquireChannel(session, requiresPaceFor(session));
            if (!holderResult) {
                qCWarning(lcPkcs15Plugin)
                    << "readTokenInfo: acquireChannel failed with code" << static_cast<int>(holderResult.error());
                return group;
            }
            holderOpt.emplace(std::move(*holderResult));
        } catch (const std::exception& ex) {
            qCWarning(lcPkcs15Plugin) << "readTokenInfo: acquireChannel threw:" << ex.what();
            return group;
        } catch (...) {
            qCWarning(lcPkcs15Plugin) << "readTokenInfo: acquireChannel threw (non-std exception)";
            return group;
        }

        auto* channel = LibreSCRS::SmartCard::Internal::HolderChannelAccessor::channel(*holderOpt);
        if (channel == nullptr) {
            qCWarning(lcPkcs15Plugin) << "readTokenInfo: channel not active after activation";
            return group;
        }
        ...
        try {
            ...
            qCWarning(lcPkcs15Plugin) << "readTokenInfo: card.readTokenInfo threw:" << ex.what();
            return group;
        } catch (...) {
            qCWarning(lcPkcs15Plugin) << "readTokenInfo: card.readTokenInfo threw (non-std exception)";
            return group;
        }
        ...
        auto safeAddText = [&](std::string_view key, std::string_view label, std::string_view value) {
            ...
            try {
                ...
                qCWarning(lcPkcs15Plugin)
                    << "readTokenInfo: addText(" << QByteArray(key.data(), static_cast<int>(key.size()))
                    << ") threw:" << ex.what();
            } catch (...) {
                qCWarning(lcPkcs15Plugin) << "readTokenInfo: addText threw (non-std exception)";
            }
        };
