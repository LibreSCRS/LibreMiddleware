...
CK_DECLARE_FUNCTION(CK_RV, C_Initialize)(CK_VOID_PTR pInitArgs)
{
    ...
    if (pInitArgs != NULL_PTR) {
        ...
        bool hasCustomMutex = (args->CreateMutex != NULL_PTR ||
    ...
    CK_UTF8CHAR_PTR pPin, CK_ULONG ulPinLen)
{
    ...
    if (!library)
        return CKR_CRYPTOKI_NOT_INITIALIZED;
    pkcs11_debug("C_Login session=%lu userType=%lu pinLen=%lu",
    ...
    CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount)
{
    ...
    if (!library)
        return CKR_CRYPTOKI_NOT_INITIALIZED;
    pkcs11_debug("C_GetAttributeValue session=%lu object=%lu attrs=%lu",
    ...
    CK_SESSION_HANDLE hSession, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount)
{
    ...
    if (!library)
        return CKR_CRYPTOKI_NOT_INITIALIZED;
    pkcs11_debug("C_FindObjectsInit session=%lu attrs=%lu",
    ...
    for (CK_ULONG i = 0; pTemplate && i < ulCount; ++i) {
        ...
        if (pTemplate[i].type == CKA_CLASS && pTemplate[i].pValue &&
            ...
            pkcs11_debug("  attr[%lu] type=0x%08lx len=%lu", (unsigned long)i,
                         (unsigned long)pTemplate[i].type, (unsigned long)pTemplate[i].ulValueLen);
        }
    }
...
{
    (void)hSession; (void)pEncryptedData; (void)ulEncryptedDataLen;
    ...
    CK_SESSION_HANDLE hSession, CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
    ...
    if (!library)
        return CKR_CRYPTOKI_NOT_INITIALIZED;
    if (pMechanism)
        pkcs11_debug("C_SignInit session=%lu mech=0x%08lx key=%lu",
    ...
    CK_BYTE_PTR pSignature, CK_ULONG_PTR pulSignatureLen)
{
    ...
    if (!library)
        return CKR_CRYPTOKI_NOT_INITIALIZED;
    pkcs11_debug("C_Sign session=%lu dataLen=%lu sigBuf=%s",
    ...
    pkcs11_debug("C_Sign -> rv=0x%08lx sigLen=%lu",
                 (unsigned long)rv,
    ...
    CK_OBJECT_HANDLE_PTR phPublicKey, CK_OBJECT_HANDLE_PTR phPrivateKey)
{
    ...
    (void)pPrivateKeyTemplate; (void)ulPrivateKeyAttributeCount;
...
{
    (void)hSession; (void)pMechanism; (void)hWrappingKey; (void)hKey;
    (void)pWrappedKey; (void)pulWrappedKeyLen;
    return CKR_FUNCTION_NOT_SUPPORTED;
}
...
CK_DECLARE_FUNCTION(CK_RV, C_DeriveKey)(
    ...
    CK_OBJECT_HANDLE_PTR phKey)
{
    (void)hSession; (void)pMechanism; (void)hBaseKey;
    (void)pTemplate; (void)ulAttributeCount; (void)phKey;
    return CKR_FUNCTION_NOT_SUPPORTED;
}
