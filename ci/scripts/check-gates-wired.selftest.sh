...
rc=$(run "$T/a")
[ "$rc" = 0 ] && grep -q 'OK: every shipped gate is wired' "$T/out"; say $? "1 green: wired + one hop"
...
rc=$(run "$T/b")
[ "$rc" = 1 ] && grep -q 'FAIL: ci/scripts/wired.sh is shipped' "$T/out"; say $? "2 red when nothing names the script"
...
rc=$(run "$T/c")
[ "$rc" = 1 ] && grep -q 'FAIL: ci/scripts/wired.sh is shipped' "$T/out"; say $? "3 a comment does not count as wiring"
...
rc=$(run "$T/d")
[ "$rc" = 1 ] && grep -q "carries no reason" "$T/out"; say $? "4 allowlist without a reason is refused"
...
rc=$(run "$T/e")
[ "$rc" = 0 ] && grep -q 'SKIP: ci/scripts/wired.sh' "$T/out"; say $? "5 allowlist with a reason is honoured"
...
rc=$(run "$T/f")
[ "$rc" = 1 ] && grep -q 'stale entry' "$T/out"; say $? "6 stale allowlist entry is refused"
...
[ "$rc" = 2 ] && grep -q 'no candidate scripts found' "$T/out"; say $? "7 an empty candidate set is 'cannot judge' (2), not pass"
